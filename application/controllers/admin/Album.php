...
class Album extends CI_Controller{
	...
	function simpan_album(){
				$config['upload_path'] = './assets/images/cover_album/'; //path folder
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
	                        ...
	                        $config['source_image']='./assets/images/cover_album/'.$gbr['file_name'];
	                        $config['create_thumb']= FALSE;
	                        $config['maintain_ratio']= FALSE;
	                        $config['quality']= '60%';
	                        $config['width']= 500;
	                        $config['height']= 400;
	                        $config['new_image']= './assets/images/cover_album/'.$gbr['file_name'];
	...
	function update_album(){
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
							...
							$path='./assets/images/cover_album/'.$images;
							unlink($path);
							$kode=$this->session->userdata('idadmin');
							$user=$this->m_pengguna->get_pengguna_login($kode);
							$p=$user->row_array();
	...
	function hapus_album(){
		...
		$path='./assets/images/cover_album/'.$gambar;
		unlink($path);
