<?php
class Konfigurasi_Admin extends CI_Controller
{
		...
		if (!empty($_FILES['filefoto']['name'])) {
			if ($this->upload->do_upload('filefoto')) {
				...
				if ($password <> $konfirm_password) {
					...
					redirect('admin/konfigurasi_admin');
				} else {
					$this->m_pengguna->simpan_pengguna($nama, $jenkel, $username, $password, $email, $nohp, $level, $photo);
					echo $this->session->set_flashdata('msg', 'success');
					redirect('admin/konfigurasi_admin');
				}
			} else {
				echo $this->session->set_flashdata('msg', 'error-tambah');
				redirect('admin/konfigurasi_admin');
			}
		} else {
			$nama = $this->input->post('xnama');
	...
	function update_pengguna()
	{
		...
		if (!empty($_FILES['filefoto']['name'])) {
			if ($this->upload->do_upload('filefoto')) {
				...
				$path = './assets/images/admin/' . $images;
				unlink($path);
				...
				if (empty($password) && empty($konfirm_password)) {
					$this->m_pengguna->update_pengguna_tanpa_pass($kode, $nama, $jenkel, $username, $password, $email, $nohp, $level, $photo);
					...
					$this->m_pengguna->update_pengguna($kode, $nama, $jenkel, $username, $password, $email, $nohp, $level, $photo);
					echo $this->session->set_flashdata('msg', 'info');
					redirect('admin/konfigurasi_admin');
				}
			} else {
	...
	function hapus_pengguna()
	{
		...
		$path = './assets/images/admin/' . $gambar;
		unlink($path);
